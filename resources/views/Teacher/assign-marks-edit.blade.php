...
<div class="card">
    <div class="card-header">
        <h3>Edit Mark on <b>{{ $course->course->name }}</b></h3>
    </div>
    <div class="card-body">
        <form action="{{ route('teacher.update.assign.marks',$marks->id) }}" method="post">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="marks" placeholder="Enter marks of this course" value="{{ $marks->marks }}">
                <span class="text-danger">{{($errors->has('marks'))? ($errors->first('marks')) : ''}}</span>
            </div>
            <div class="form-group">
                <button class="btn btn-success btn-block">Update Mark</button>
            </div>
        </form>
    </div>
</div>
